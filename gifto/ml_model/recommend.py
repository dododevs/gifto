...
def main():
	...
	recommend(user_age, user_gender, user_hobbies)


def recommend(user_age, user_gender, user_hobbies):
	...
	products_df = pd.read_csv(MODEL_DIR + "/final_dataset.csv")
	log("[INFO] Products data loaded. Rows:", len(products_df))

	# 4) Load encoders/scalers used during training
	gender_encoder = joblib.load(MODEL_DIR + "/gender_encoder.pkl")
	cat_encoder = joblib.load(MODEL_DIR + "/category_encoder.pkl")
	hobby_encoder = joblib.load(MODEL_DIR + "/hobby_encoder.pkl")
	price_scaler = joblib.load(MODEL_DIR + "/price_scaler.pkl")
	stars_scaler = joblib.load(MODEL_DIR + "/stars_scaler.pkl")
	hci_encoder = joblib.load(MODEL_DIR + "/hobbies_category_interaction_encoder.pkl")
	...
	# 	log(f"{p_id}\t{title}...\tlike probability: {score_pct:.2f}%")
	...
	# log(top_candidates[["product_id", "score"]])

	# Debug prints
	# log("\n(DEBUG):\n", candidates[["hobbies_encoded", "category_encoded", "hobbies_category_interaction"]].head(10))
	return list(candidates.nlargest(5, "score")["product_id"])
